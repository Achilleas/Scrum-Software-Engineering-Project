...
public class JettyServer {
	...
	public void run(String[] args) {
		...
		handler2.addServlet(new ServletHolder(new CheckUsername()),
				"/user-check");
		handler2.addServlet(new ServletHolder(new ProfilePage()), "/profile");
		handler2.addServlet(new ServletHolder(new SignOut()), "/signout");
		handler2.addServlet(new ServletHolder(new MarketOverview()),
				"/overview");
		handler2.addServlet(new ServletHolder(new RefreshOverview()),
				"/refreh-overview");
		handler2.addServlet(new ServletHolder(new StockChooser()), "/stocks");
		handler2.addServlet(new ServletHolder(new RefreshStockChooser()),
				"/refresh-stocks");
		handler2.addServlet(new ServletHolder(new VisShare()), "/share-vis");
		handler2.addServlet(new ServletHolder(new VisAllShare()),
				"/all-share-vis");
