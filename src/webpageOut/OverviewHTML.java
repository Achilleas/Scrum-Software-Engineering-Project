...
public class OverviewHTML extends WriteOut {
	...
	private void writeStocks(Investor ip) {
		NavigableSet<String> ftseList = FinanceQuery.getComponentsList(Constants.FTSE100);
		...
		while(it.hasNext()){
			sym = it.next();
			LinkedList<Stock> list = FinanceQuery.getLatestPrice(sym);
			stock = list.get(0);
			//% Change of Current from Opening
			double percent = (stock.getLatest()-stock.getOpen())/stock.getOpen();
			...
			String interested = "";
			if(ip.isInterested(sym)||ip.isInvested(sym)){
